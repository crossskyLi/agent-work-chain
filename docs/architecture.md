...
**区块链设施：** 自有合约 `TrustChain.sol`，通过 Hardhat 部署到 Base (L2)。
...
仲裁通过 `TrustChain.sol` 对接 Kleros（ERC-792）处理：

- 任一任务参与方都可发起争议
- Kleros 裁决后回调 `rule()` 完成链上结算
- 裁决结果链上不可篡改
...
`TrustChain.sol` 内置协议费配置，用于 Trust 服务收费：

- `feeBps`：手续费比例（基点，默认 10 = 0.1%）
- `feeCapWei`：单笔手续费封顶
- `feeRecipient`：手续费接收钱包

结算时手续费计算：

```
fee = min(reward * feeBps / 10000, feeCapWei)
net = reward - fee
```

可由合约 owner 动态调整：

`setFeeConfig(feeRecipient, feeBps, feeCapWei)`
